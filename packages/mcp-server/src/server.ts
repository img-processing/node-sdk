...
import { ClientOptions } from 'img-processing';
...
export { ClientOptions } from 'img-processing';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'img_processing_api',
    version: '1.4.0',
  },
  {
    capabilities: {
...
export function initMcpServer(params: {
  ...
  const client = new ImgProcessing(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client = params.client || new ImgProcessing({ defaultHeaders: { 'X-Stainless-MCP': 'true' } });
